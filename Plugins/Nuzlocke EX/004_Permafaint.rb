...
# Override the configured revive item handler to allow reviving perma-fainted Pokémon when MODOASISTIDO is active
# The symbol is read from config at load time, but the handler logic runs at runtime
_modoasistido_revive_item = ChallengeModes::RULES[:MODOASISTIDO][:revive_item]
ItemHandlers::UseOnPokemon.add(_modoasistido_revive_item, proc { |item, qty, pkmn, scene|
  # If MODOASISTIDO is active, allow reviving perma-fainted Pokémon
  if ChallengeModes.on?(:MODOASISTIDO)
    if !pkmn.fainted?
      scene.pbDisplay(_INTL("No tendría ningún efecto."))
      next false
    end
    pkmn.perma_faint = false if pkmn.perma_faint
    pbSEPlay("Use item in party")
    pkmn.heal_HP
    pkmn.heal_status
    scene.pbRefresh
  ...
  # Default behavior - block perma-fainted Pokémon
  if !pkmn.fainted? || pkmn.perma_faint
    scene.pbDisplay(_INTL("No tendría ningún efecto."))
    next false
  end
